...
def callback_buttons(call):
    ...
    bot.send_message(call.from_user.id, "А теперь опиши проблему, которую ты заметил и пришли её фото.")
    place = call.data


@bot.message_handler(func=lambda message: get_user_step(message.from_user.id) == 2)
def stage_two(message):
    bot.send_message(message.from_user.id, "Ваше сообщение принято и отправлено в Роскомнадзор, спасибо.")
